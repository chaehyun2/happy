...
describe('PermissionHandler', () => {
    ...
    it('uses agentID to disambiguate sub-agent permission requests with the same toolUseID', async () => {
        ...
        const firstPending = handler.handleToolCall(
            ...
            { command: 'pwd' },
            mode,
            { signal: firstController.signal, requestId: 'req_toolu_shared_a', toolUseID: 'toolu_shared', agentID: 'agent-a' },
        );
        const secondPending = handler.handleToolCall(
            'Bash',
            { command: 'whoami' },
            mode,
            { signal: secondController.signal, requestId: 'req_toolu_shared_b', toolUseID: 'toolu_shared', agentID: 'agent-b' },
        );
